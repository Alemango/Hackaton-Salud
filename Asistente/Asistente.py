...
import os #Librería para el manejo del sistema operativo
...
def SelectorAccion(ResBody, IntencionJSON):
    ...
    elif IntencionJSON['intent'] == 'Medicamento':
        try: 
            consultaMedicamento()
        except:
            T2SError()
...
#Función para consulta de medicamentos a través de imagen
def getTextByImage():
    ...
    path = (r"C:\Users\aleal\OneDrive\Documentos\Prog\pruebaImage2Text")

    if leido == True:
        cv2.imwrite(os.path.join(path,nombreFoto), frame)
    ...
    if read_result.status == OperationStatusCodes.succeeded:
        for text_result in read_result.analyze_result.read_results:
            for line in text_result.lines:
                medicine = line.text
                # print(line.text)
                r = requests.get('https://medicamentoapi-default-rtdb.firebaseio.com/{medicine}.json'.format(medicine=medicine))
...
def consultaMedicamento():
    T2S("Coloca el medicamento frente a la cámara, enfocado en el ingrediente activo.")
    variable = getTextByImage()
    variable = json.loads(variable)
    general = 'El medicamento ' + str(variable['nombre']) +' vía ' + str(variable['via']) + '. ' + str(variable['generalidades'] + '. Está indicado principalmente para ' + str(variable['indicaciones'][0]))
    T2S(general)
